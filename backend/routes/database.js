...
const pullArticlesFromDB = async (page, limit) => {
    try {
        ...
        return page && limit ? await query.skip((page - 1) * limit).limit(limit).toArray() : await query.toArray();
    } catch (err) {
    ...
    try {
        ...
        for (const article of articles) {
            if (article.description === "") {
                ...
                await mongoose.connection.db.collection('articles').updateOne({_id: article._id}, {$set: {description: article.description}});
...
const verifyContent = async () => {
    ...
    console.log("Finished verifying the content of the articles in the database.");
}

connectToDB().catch(err => console.error("Error connecting to the database:", err.stack));
verifyContent().catch(err => console.error("Error verifying content:", err.stack));

module.exports = {connectToDB, pushArticlesToDB, pullArticlesFromDB, removeDupesFromDB};