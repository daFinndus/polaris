...
const AudioButton = () => {
  ...
  return (
    <div className="flex notebook:w-12 w-1/2 h-12">
      <Button
        variant="secondary"
  ...
  return (
    <div className={"relative w-full p-8 h-3/4"}>
      <video
        ...
        ref={videoRef}
        className={`brightness-60 blur-[2px] rounded-2xl object-cover w-full h-full transition-opacity duration-500 ${fade ? "opacity-0" : "opacity-100"}`}
        src={currentVideo}
        autoPlay
        muted
        playsInline
      />
      <p
        className={`text-[25vw] drop-shadow-2xl h-min absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 text-center font-bold opacity-100 transition-colors duration-500 ${fade ? "text-primary" : "text-background"}`}>404</p>
    </div>
  );
};
...
const Supported = () => {
  ...
  return (
    <div className={"flex flex-col h-screen w-screen items-center bg-background justify-center font-sans"}>
      <div
        className={"flex gap-x-2 notebook:w-min justify-center items-center fixed top-4 right-4"}>
        <HomeButton />
        <AudioButton />
      </div>
      <VideoBackground />
