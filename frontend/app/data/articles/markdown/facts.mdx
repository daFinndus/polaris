...
Adding `facts.htb` to `/etc/hosts` and browsing to the web server on port 80 returns a fairly bare site, nothing immediately useful. That's when I decided to use `ffuf` to fuzz for hidden directories:
