...
Starting off with an `nmap` scan reveals three open ports:

<Block language="Bash" content={"22 - SSH\n80 - HTTP\n54321 - (unknown service)"} />

Adding `facts.htb` to `/etc/hosts` and browsing to the web server on port 80 returns a fairly bare site, nothing immediately useful. That's wenn I decided to use `ffuf` to fuzz for hidden directories:
...
A bit of research gave me a privilege escalation script <Marker content="privilege escalation script" link="https://github.com/ch3m1cl/Facter-PrivEsc" />. It works by creating a custom ruby configuration for <Code content="facter" /> that executes a reverse shell when the binary is run.
