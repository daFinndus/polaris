...
<Bild
  src={
    "https://s8lcpnzirhyz4bbt.public.blob.vercel-storage.com/Blogs/wingdata-website.png"
  }
...
<Bild
  src={
    "https://s8lcpnzirhyz4bbt.public.blob.vercel-storage.com/Blogs/wingdata-login.png"
  }
...
<Block
  language="Bash"
  content={"hashcat -m 1410 salts.hash /usr/share/wordlists/rockyou.txt"}
/>
...
<Block
  language="Bash"
  content={"python3 CVE-2025-4138.py -u wacky -o backup_1011.tar"}
/>
...
<Block
  language="Bash"
  content={
    "sudo python3 /opt/backup_clients/restore_backup_clients.py -b backup_1011.tar -r restore_wacky"
  }
/>
...
Short box, but with some solid lessons baked in, especially around how software handles secrets and archive extraction.

<br />

**Static salts are as good as no salt**: Wing FTP used <Code content="WingFTP" /> as a hardcoded salt for every password hash. When hashcat doesn't crack unsalted hashes, I should remember to check for static salts. If the salt is static and known, cracking is just as easy as with unsalted hashes.

<br />

**The exploit directory is worth digging through**: Landing in <Code content="/opt/wingftp" /> and finding XML files with hashes wasn't immediately obvious, but taking the time to search through the installation directory paid off directly.

<br />

**Filename validation can be bypassed at the tar level**: Just like zip files, tar archives can contain entries that bypass application-layer filename checks. Whenever I see a root-executed script extracting archives, I should look for a tarball path traversal CVE.
